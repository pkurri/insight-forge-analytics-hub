...
const BusinessRules: React.FC<BusinessRulesProps> = ({ datasetId, validationComplete }) => {
  ...
  const handleConfirmDeleteRule = async () => {
    ...
    setIsLoading(true);
    try {
      // Always use the dataset-scoped delete endpoint
    ...
    <Tabs defaultValue="rules" className="w-full">
      ...
      <TabsContent value="rules">
        <div className="mb-4 flex gap-2">
          <Button onClick={() => setShowAddRuleDialog(true)} variant="outline">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button onClick={handleGenerateRule} disabled={isLoading || !validationComplete}>
              {isLoading ? (
                <><RefreshCw className="mr-2 h-4 w-4 animate-spin" /> Generating Rules...</>
              ) : (
                <><Sparkles className="mr-2 h-4 w-4" /> Generate Rules with ML</>
              )}
            </Button>
          </div>
          <Button onClick={handleSaveRules} variant="secondary">
            ...
            <Card key={rule.id} className="border p-2">
              <CardHeader className="flex flex-row items-center justify-between">
                ...
                <div className="flex gap-2">
                  ...
                  {rule.model_generated && <Badge variant="outline">AI</Badge>}
                  {rule.source && <Badge variant="secondary" className="capitalize">{rule.source.replace('_', ' ')}</Badge>}
                </div>
              </CardHeader>
              <CardContent>
                ...
                <div className="flex gap-2 mt-2">
                  <Button size="sm" variant="outline" onClick={() => handleToggleRule(rule.id)} disabled={isLoading}>
                    {rule.active ? <Check className="h-4 w-4 mr-1" /> : <X className="h-4 w-4 mr-1" />}
                    {rule.active ? "Disable" : "Enable"}
                  </Button>
                  <Button size="sm" variant="outline" onClick={() => handleEditRule(rule)} disabled={isLoading}>
                    <svg className="h-4 w-4 mr-1" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M15.232 5.232l3.536 3.536M9 13l6.536-6.536a2 2 0 112.828 2.828L11.828 15.828A4 4 0 017 17H5v-2a4 4 0 011.172-2.828z"/></svg>
                    Edit
                  </Button>
                  <Button size="sm" variant="outline" onClick={() => confirmDeleteRule(rule)} disabled={isLoading}>
        ...
        {/* Review Dialog for AI-generated rules */}
        <Dialog open={showGeneratedDialog} onOpenChange={setShowGeneratedDialog}>
          <DialogContent className="max-w-lg">
            <DialogHeader>
              <DialogTitle>Review AI-Generated Business Rules</DialogTitle>
              <DialogDescription>
                Review and accept the following rules generated by AI. You may edit or discard them before saving.
              </DialogDescription>
            </DialogHeader>
            <div className="max-h-80 overflow-y-auto space-y-4 py-2">
              {generatedRules.length === 0 ? (
                <div className="text-muted-foreground">No rules generated.</div>
              ) : generatedRules.map((rule, idx) => (
                <Card key={rule.id || idx} className="border p-2">
                  <CardHeader className="flex flex-row items-center justify-between">
                    <div className="font-semibold">{rule.name}</div>
                    <div className="flex gap-2">
                      {rule.model_generated && <Badge variant="outline">AI</Badge>}
            ...
            <DialogFooter>
              ...
              <Button
                onClick={() => {
                  ...
                  setGeneratedRules([]);
                  toast({ title: "Rules Added", description: `${generatedRules.length} AI-generated rules merged.` });
                }}
        ...
        {/* Edit Rule Dialog */}
        <Dialog open={editDialogOpen} onOpenChange={val => { if (!val) setEditRule(null); setEditDialogOpen(val); }}>
          <DialogContent className="max-w-md">
            <DialogHeader>
              <DialogTitle>Edit Business Rule</DialogTitle>
              <DialogDescription>
                Update the details of your business rule.
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-4 py-2">
              <div className="space-y-2">
